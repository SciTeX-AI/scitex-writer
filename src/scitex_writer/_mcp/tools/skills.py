...
def _import_failure_reason() -> str:
    """Why the scitex-dev skills API could not be imported, stated precisely.

    An ``except ImportError`` here catches two different failures and cannot
    tell them apart: scitex-dev is ABSENT, or it is PRESENT and the symbol
    moved. These handlers reported the first unconditionally, so when
    ``scitex_dev.skills`` was folded into ``scitex_dev.ecosystem`` both tools
    began answering "scitex-dev not installed" with the package sitting right
    there — a message that sends the reader to `pip show scitex-dev`, where
    they find it installed and go debug something else.

    The remedy differs too: install it, versus update the call site. Naming the
    wrong one costs more than naming none.
    """
    if importlib.util.find_spec("scitex_dev") is None:
        return "scitex-dev is not installed"
    return (
        "scitex-dev is installed but does not expose its skills API where "
        "scitex-writer looks for it (scitex_dev.ecosystem.list_skills / "
        ".get_skill) — the symbol has moved; scitex-writer needs updating"
    )
...
def register_tools(mcp: FastMCP) -> None:
    ...
    def writer_skills_list() -> dict:
        ...
        try:
            ...
            return {"success": False, "error": _import_failure_reason()}
